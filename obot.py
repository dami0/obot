...
class play_char:
    '''
    This is the class for the player characters, containing all the
    information about a player sans the description.
    '''
    player_count = 0
    ...
    def physical(self):
        output = ', 'join.(self.race, self.age, self.hght, self.wght, self.sex)
        return output

    def attributes(self):
        output = []
        for atr in self.attr:
            output.append(atr + ": " + self.attr(atr))
        output = ', '.join(output)
        return output

    def current(self):
        output = ', '.join(self.gold, self.hp, self.xp, self.ac)
        return output

# Reads in a JSON character sheet (generated by a Perl script, see
# https://github.com/kirbyman62/osric-character-sheet-to-json)
# and returns a player object with the details
